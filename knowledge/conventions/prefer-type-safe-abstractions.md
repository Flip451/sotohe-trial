...
> **強制先**: review 観点 — types / entities / use_cases / frameworks / web / web_composition / interface_adapters scope
...
1. **標準ライブラリ / 既存 crate で排除可能か?**
   ...
   > **強制先**: review 観点 — entities / use_cases / frameworks / web scope
...
2. **型で表現可能か?**
   ...
   > **強制先**: review 観点 — types / entities / use_cases / frameworks / web / interface_adapters scope
...
| 状態が永続化から復元される (serde 必要) | → entities 層は **typestate**、frameworks 層で serde 対応 enum DTO に変換 (ヘキサゴナル分離) |
| 状態数が多く組み合わせ爆発する | → enum + runtime validation (typestate の型爆発を避けるエスケープハッチ) |

> **強制先**: review 観点 — types / entities / frameworks scope
...
**永続化が必要な場合：**
entities 層では typestate を維持し、frameworks 層で serde 対応 enum DTO と相互変換する。
- domain → DTO: `From<Order<Shipped>> for OrderDto`
- DTO → domain: `TryFrom<OrderDto> for Order<Placed>` (fallible — 不正な状態復元は `Result` で報告)

> **強制先**: review 観点 — types / entities / frameworks scope

entities 層の型安全性を永続化の都合で妥協しない (ヘキサゴナルアーキテクチャの原則)。

> **強制先**: review 観点 — entities / frameworks scope
...
- [ ] 状態遷移がある場合、typestate パターンを検討したか
  > **強制先**: review 観点 — types / entities / use_cases / frameworks / web / interface_adapters scope
- [ ] serde が必要な場合、entities 層の typestate は維持されているか (frameworks 層で DTO 変換)
  > **強制先**: review 観点 — types / entities / frameworks scope
