...
> **強制先**: review 観点 — entities / use_cases / frameworks / web / web_composition / interface_adapters scope
...
### `interface_adapters` と `web`

HTTP コントローラでは route / status / body / media-type の境界と、DTO↔Command/Query 変換
（happy path と不正入力の拒否）をテストする。薄い `web` bin ではサーバ lifecycle outcome の
提示と exit code だけを確認する。use_cases や frameworks の内部規則を再テストするのではなく、
adapter 間のデータとエラーの受け渡しを確認する。

> **強制先**: review 観点 — web / interface_adapters scope
...
> **強制先**: review 観点 — web_composition scope
...
> **強制先**: review 観点 — use_cases / frameworks / web / interface_adapters scope

evaluator には、生成した入力と状態について、仕様の不変条件を保った判定、結果の一貫性、および失敗時
の結果を確かめる property-based test を置く。

> **強制先**: review 観点 — entities / use_cases / frameworks scope
...
- [ ] codec、parser、evaluator の変更に property-based test があり、対象の約束へ binding されている
  > **強制先**: review 観点 — entities / use_cases / frameworks / web / interface_adapters scope
