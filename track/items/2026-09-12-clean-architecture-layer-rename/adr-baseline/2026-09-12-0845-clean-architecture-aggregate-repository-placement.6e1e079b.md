...
- Aggregate-root classification for `User` / `AccessTokenRecord` changes, or those aggregates leave `entities`.
- Catalogue-lint or type-designer policy stops treating `Repository` as innermost-only.
- A second bounded context needs shared persistence ports that cannot stay beside a single aggregate.
