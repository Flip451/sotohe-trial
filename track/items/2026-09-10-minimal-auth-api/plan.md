...
- [x] **T003**: Implement the usecase entries `RegisterUserService`, `RegisterUserInteractor`, and `RegisterUserError`, plus result tests (IN-001, CN-001, CN-002, CN-005, AC-001, AC-003).
...
- [x] **T006**: Implement the cli_driver entries `AuthHttpApi` and `HttpServerOutcome` as the shared axum HTTP foundation, plus focused server lifecycle, JSON rejection, and HTTP outcome tests (IN-001, IN-002, CN-003, CN-004).
