# Use_cases Layer Review: Severity Policy

The reviewer's role is **purity and orchestration correctness review** of
`libs/use_cases/`. The use_cases layer is a **pure orchestrator** — it composes
domain ports and use_cases ports into application flows; it must never reach
...
- **purity violation by trait or generic**: a `T: Reader` bound that effectively
  ...
  a re-export), or a generic constraint that lets frameworks leak into
  use_cases. Name the runtime dependency the bound admits and the re-export path
...
- **implicit time / env / process dependency**: a function that reads
  ...
  User-supplied time belongs in the use_cases entrypoint; execution time,
  randomness, and generated identifiers must be acquired through explicit
  use_cases-owned secondary ports whose runtime adapters are injected.
- **business logic leak**: a calculation, branching, or decision that belongs
  ...
  Newtype) executed in use_cases. The boundary runs the other way here: use_cases
  orchestrates, domain decides. Name the domain type the decision belongs on.
- **port placement mistake**: a port defined in use_cases that should live in
  entities (a port abstracting an entities concept, not an external
  capability). Name the concept the port abstracts.
- **direct frameworks reference**: any non-test code in use_cases importing
  from `frameworks::*` (even via re-export). Cite `architecture-rules.json`.
- **error type confusion**: the use_cases error enum re-exposes frameworks
  error variants (e.g., `io::Error`) instead of mapping them to a
  use_cases-level concept, breaking the abstraction. The interactor's
  callers should not need to know an `io::Error` could happen.
- **output side-effect in use_cases**: `println!` / `eprintln!` / file write /
  TCP send inside a use_cases function. Outputs belong in the delivery mapping.
