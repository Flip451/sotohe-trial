# Frameworks Layer Review: Severity Policy

The reviewer's role is **adapter correctness and I/O boundary review** of
`libs/frameworks/`. Frameworks is where ports get their adapters, owns the
HTTP server lifecycle, and where external I/O actually happens (file system,
processes, network, git, JSON / TOML / YAML codecs). The reviewer must catch issues that compile
...
- **fingerprint or hash drift**: a hash / fingerprint function over a struct
  ...
  encoding.
- **layer leak**: code in frameworks that calls into `apps/web` /
  `apps/web-composition` (reverse-direction dependency, violates
