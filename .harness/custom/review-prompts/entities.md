# Entities Layer Review: Severity Policy

The reviewer's role is **type-level / invariant-level correctness review** of
`libs/entities/`. The entities layer is the innermost layer with **zero
...
- **port misplaced**: a trait that abstracts a non-domain technical capability
  (git hash, two-phase commit, HTTP client) placed in entities instead of
  use_cases. Do **not** flag aggregate `Repository` ports — those stay in
  `libs/entities` per the Repository lint constraint and binding adjudication.
  Name the capability the trait abstracts when reporting.
- **purity violation**: `std::fs::*`, `std::net::*`, `std::process::*`,
  `std::io::*`, `std::env::*`, `chrono::Utc::now()`, `std::time::SystemTime`,
  `std::time::Instant`, `println!` / `eprintln!` reaching entities (these are
  caught by `sotp verify usecase-purity` for use_cases but should never appear in entities
  either by extension).
...
- Adding derives (`Clone` / `Hash` / `Display`) that the catalogue contract
  intentionally omits — verify catalogue first via `<track>/entities-types.json`
...
- Only for track `scope-conditional-pre-review-gates-2026-07-31`: the layer placement of
  ...
  `libs/entities/src/review_v2/ports.rs` (introduced by an earlier track under the
