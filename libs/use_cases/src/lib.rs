...
//! and depends **only** on the [`entities`] crate. Outward dependencies (I/O,
//! persistence, external services) are expressed as *ports* — traits defined
//! here and implemented by adapters in the frameworks layer.
//!
...
// Re-export the entity types that appear in this layer's public API so that
// downstream layers depending only on `use_cases` (e.g. the CLI driver) can name
// them without taking a direct dependency on `entities`.
pub use entities::auth::OpaqueAccessToken;
pub use entities::{Username, UsernameError};
...
///
/// The use_cases layer owns this contract; an infrastructure adapter provides the
...
pub mod auth {
    ...
    use entities::{Username, UsernameError};
    ...
    mod tests {
        ...
        impl UserRepository for RegistrationUserRepository {
            ...
            fn save(&self, user: User) -> Result<(), entities::auth::UserRepositoryError> {
                if self.duplicate_on_save {
                    return Err(entities::auth::UserRepositoryError::DuplicateUser);
                }
                if self.save_unavailable {
                    return Err(entities::auth::UserRepositoryError::Unavailable);
                }
        ...
        fn test_register_user_interactor_persistence_failure_returns_persistence_error() {
            ...
            assert_eq!(
                interactor.execute(valid_registration_command()),
                Err(RegisterUserError::Persistence(
                    entities::auth::UserRepositoryError::Unavailable
                ))
            );
        }
        ...
        impl UserRepository for FixedUserRepository {
            ...
            fn save(&self, _user: User) -> Result<(), entities::auth::UserRepositoryError> {
                Ok(())
            }
        }
